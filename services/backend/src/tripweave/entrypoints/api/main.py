...
def create_app(settings: Settings | None = None, engine: Engine | None = None) -> FastAPI:
    ...
    def list_trip_map_points(
        ...
        statement = (
            ...
            .join(orm.TripMember, orm.TripMember.trip_id == orm.Trip.id)
            .join(orm.TripMapPointProjection, orm.TripMapPointProjection.trip_id == orm.Trip.id)
            .where(
                orm.TripMember.user_id == auth.user.id,
                orm.TripMember.removed_at.is_(None),
            )
            .order_by(
                ...
                orm.Trip.id,
                orm.TripMapPointProjection.captured_at.asc().nulls_last(),
                orm.TripMapPointProjection.media_item_id,
            )
        )
        if start is not None:
            statement = statement.where(
                orm.TripMapPointProjection.captured_date.is_not(None),
                orm.TripMapPointProjection.captured_date >= start,
            )
        if end is not None:
            statement = statement.where(
                orm.TripMapPointProjection.captured_date.is_not(None),
                orm.TripMapPointProjection.captured_date <= end,
            )

        points: list[TripMapPointResponse] = []
        trips_by_id: dict[UUID, TripMapSummary] = {}
        for trip, role, member_id, projection in db.execute(statement).all():
            point_latitude = projection.latitude
            point_longitude = projection.longitude
            captured_at = projection.captured_at
            point = TripMapPointResponse(
                id=projection.media_item_id,
                tripId=trip.id,
                mediaItemId=projection.media_item_id,
                capturedAt=captured_at,
                date=projection.captured_date,
            ...
            if summary is None:
                summary = TripMapSummary(
                    ...
                    representative_media_item_id=projection.media_item_id,
                )
        ...
        response = TripsMapPointsResponse(
            trips=[
                TripMapTripResponse(
                    id=summary.trip.id,
